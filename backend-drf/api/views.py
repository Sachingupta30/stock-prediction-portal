...
class StockPreditionAPIView(APIView):
    def post(self, request):
        ...
        if serializer.is_valid():
            ...
            print(df)
            if df.empty: #check for invalid ticker
                return Response({"error": "No data found for the given ticker.",
                                 "status": status.HTTP_404_NOT_FOUND})
            
            """
                Resetting the index so that the actual row numbers become the default index,
                instead of using column names as the index.
            """
            df = df.reset_index() 
